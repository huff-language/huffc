...
parser.processMacro = (
  ...
  result.tableInstances.forEach((tableInstance) => {
    ...
    bytecode = `${pre}${padNBytes(
      formatEvenBytes(toHex(tableOffsets[tableInstance.label])),
      2
    )}${post}`;
  });
...
parser.parseTopLevel = (raw, startingIndex, inputMap) => {
  ...
  while (!regex.endOfData(input)) {
    if (currentContext === CONTEXT.NONE && input.match(grammar.topLevel.TEMPLATE)) {
      ...
      currentContext & (CONTEXT.MACRO | CONTEXT.NONE) &&
      grammar.topLevel.MACRO.test(input)
    ) {
      ...
      if (name.endsWith("_LOCATION")) {
        ...
        if (storageLocations.includes(value)) {
          errors.push(`Storage location 0x${value} for macro ${name} already exists`);
        }
  ...
  const recurse = (filename, parentPath) => {
    ...
    if (!fileString) {
      if (!fs.existsSync(relativePath)) {
        throw new Error(
          `Import not found: ${fileName} ${
            parentPath && parentPath !== "." ? `in ${parentPath}` : ""
          }`
        );
      }
...
parser.getContents = (source, _path) => {
  ...
  const sources = Object.keys(source).reduce(
    (obj, key) => ({
      ...obj,
